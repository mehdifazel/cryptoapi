/**
 * Ramzinex Live Prices - Main app. Feature parity with Nobitex (RLS/USDT markets, orderbook, setup, TV).
 */
(function() {
  'use strict';
  var C = window.RamzinexConfig || {};
  var Api = window.RamzinexApi || {};
  ...
  var state = {
    filter: 'rls', sortCol: null, sortDir: 1, networkFilter: '', stats: {}, prevStats: {}, pairIds: {},
    networkDb: null, currentOrderbookBase: null, tvPrices: {}, globalAvgBuyMT: null, globalAvgSellMT: null,
  ...
  function getPairId(pair) { return state.pairIds && state.pairIds[pair]; }
  ...
  function withdrawEnabled() { return true; }
  ...
  function updateUsdtAvg(onDone) {
    ...
    var pid = getPairId('usdt-rls');
    if (pid == null) { setEl($.usdtAvgBuy100MT, '-'); setEl($.usdtAvgSell100MT, '-'); if (typeof onDone === 'function') onDone(); return; }
    Api.fetchOrderbook(pid).then(function(d) {
  ...
  function showOrderbook(pair) {
    ...
    if (pairId == null) {
      ...
      $.orderbookAsksBody.innerHTML = '<tr><td colspan="3">Market not found.</td></tr>';
      $.orderbookBidsBody.innerHTML = '';
      $.orderbookBody.style.display = 'flex';
      return;
    }
    Api.fetchOrderbook(pairId).then(function(d) {
      $.orderbookLoading.style.display = 'none';
      if (d.status !== 'ok' || !d.asks || !d.bids) {
        $.orderbookAsksBody.innerHTML = '<tr><td colspan="3">No data for this market.</td></tr>';
        $.orderbookBidsBody.innerHTML = '';
      } else if (!d.asks.length && !d.bids.length) {
  ...
  function updateSetup() {
    ...
    var tbody = $.tbody;
    var pairs = Object.keys(state.stats).filter(function(k) { return k.indexOf('-rls') !== -1 && withdrawEnabled() && hasPriceData(state.stats[k]); }).map(function(k) { return [k, state.stats[k]]; }).sort(function(a, b) { return (b[1].volumeDst || 0) - (a[1].volumeDst || 0); });
    ...
    function next() {
      ...
      var pid = getPairId(rlsPair); if (pid == null) { setTimeout(next, C.SETUP_DELAY_MS || 250); return; }
      Api.fetchOrderbook(pid).then(function(d) {
  ...
  function updateSetupUsdt() {
    ...
    function next() {
      ...
      if (!row) { setTimeout(next, 0); return; }
      var pid = getPairId(pair); if (pid == null) { setTimeout(next, C.SETUP_DELAY_MS || 250); return; }
      Api.fetchOrderbook(pid).then(function(d) {
        if (d.status !== 'ok' || !d.asks || !d.bids) return;
  ...
  function hasNetwork(base, netFilter, networkDb, parseBaseMultiplier) {
    return Ndb.hasNetwork ? Ndb.hasNetwork(base, netFilter, networkDb, parseBaseMultiplier) : true;
  }
  ...
  function renderTable(stats) {
    ...
    if (state.filter === 'rls') {
      ...
      var pairs = Object.keys(stats).filter(function(k) { return k.indexOf('-rls') !== -1 && withdrawEnabled() && hasPriceData(stats[k]) && hasNetwork(k.slice(0, -4).toLowerCase(), netFilter, state.networkDb, parseBaseMultiplier); }).map(function(k) { return [k, stats[k]]; }).sort(function(a, b) { return (b[1].volumeDst || 0) - (a[1].volumeDst || 0); });
      ...
      if (!tbody.querySelector('tr[data-rls-pair]') || filterChanged) {
        tbody.innerHTML = pairs.map(function(item) { var rlsPair = item[0], s = item[1], base = rlsPair.slice(0, -4), m = base.toUpperCase(), dc = (s.dayChange != null && s.dayChange !== '') ? String(s.dayChange) : '', tags = getPairTagsHtml(base), nets = getNetworksShort(base); return '<tr data-rls-pair="'+rlsPair+'" data-networks="'+nets+'" data-day-change="'+dc+'" onclick="event.stopPropagation(); window.RamzinexApp.showOrderbook(\''+rlsPair+'\')"><td class="pair">'+m+tags+'</td><td class="price" data-field="rls-bestBuy"></td><td class="price" data-field="rls-bestSell"></td><td class="price" data-field="rls-latest"></td><td class="positive" data-field="rls-dayChange"></td><td class="price" data-field="rlsSellSetup"></td><td class="price" data-field="rlsBuySetup"></td><td class="price" data-field="usdtSell"></td><td class="price" data-field="usdtBuy"></td><td class="price" data-field="tvPrice"></td></tr>'; }).join('');
      }
  ...
  function pollStats() {
    Api.fetchMarkets().then(function(r) {
      if (r && r.stats && Object.keys(r.stats).length) {
        state.stats = r.stats; if (r.pairIds) state.pairIds = r.pairIds;
  ...
  function initRest() {
    ...
    Api.fetchMarkets().then(function(r) {
      if (r && r.stats && Object.keys(r.stats).length) {
        state.stats = r.stats; state.pairIds = r.pairIds || {};
        state.prevStats = {};
      ...
      else { showError('Connection error. The API may not allow browser requests (CORS). Ask the exchange to enable CORS for public endpoints; or try another network/VPN.'); state.restRetry = setTimeout(initRest, 5000); }
    });
  }

  window.RamzinexApp = { showOrderbook: showOrderbook };
